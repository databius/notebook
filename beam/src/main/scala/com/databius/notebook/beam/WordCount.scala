...
object WordCount {
  def main(args: Array[String]): Unit = {
    ...
    pipeline
      .apply("ReadFiles", TextIO.read().from(options.getInputFile))
...
trait WordCountOptions extends PipelineOptions {
  ...
  //  @Default.String("gs://ghpr-dev-dataflow-staging-us/temp")
  @Default.String("./data/output/wordcount")
...
class CountWords
    extends PTransform[PCollection[String],
                       PCollection[KV[String, java.lang.Long]]] {
  override def expand(
      input: PCollection[String]): PCollection[KV[String, lang.Long]] = {
    input
      .apply(ParDo.of(new ExtractWords)) //Ignore IntelliJ error: "Cannot resolve apply". The code will compile.
      .apply(Count.perElement())
  }
}
